...
public class Scrutiny_MarkDialog_R13_BTech_SpecialCase_New extends Activity implements OnClickListener,
OnLongClickListener , TextWatcher {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		R13BTech = instanceUtitlity
				.isRegulation_R13_Btech(Scrutiny_MarkDialog_R13_BTech_SpecialCase_New.this);
	...
	private void showItems() {
		 ...
		 */
		 ...
		 //Swapna
					// PART-A --> BEST OF 3 FROM 4 QUES [10X3=30M]; PART-B
					// --> 5 QUES IS COMPULSORY [1X45=45M] GRAND_TOTAL = 75M
	...
	private void showRemarksInETWithGreenColor(String C_REMARK,
		...
		if (!TextUtils.isEmpty(C_REMARK)) {
			...
			PeditText.setOnClickListener(Scrutiny_MarkDialog_R13_BTech_SpecialCase_New.this);
	...
	protected Boolean setSampleRemark(EditText mEditText) {
		...
		if (text.equals("") && tag.equals("")) {
			...
			if (mEditText.getCurrentTextColor() == getResources().getColor(
				...
				mEditText.setOnClickListener(Scrutiny_MarkDialog_R13_BTech_SpecialCase_New.this);
				check = true;
				}else{
	...
	public void onClick(View view) {
		 ...
		 * checkRemarkCountForShowingAlert(); } else {
	...
	private boolean checkGroundTotal(String grand) {
		 ...
		 * if(!TextUtils.isEmpty(ss)){ float roundOffGrandTotal1
		 * =Float.valueOf(ss); if(!TextUtils.isEmpty(grand)){ float
		 * roundOffGrandTotal = Float.valueOf(grand); Log.e("grand",
		 * roundOffGrandTotal+" "+roundOffGrandTotal1);
		 * if(roundOffGrandTotal1==roundOffGrandTotal){ return true; }else{
		 * alertMessageForMarksMisMatch(ss); return false; } }else{
		 * alertMessageForCharge(getString(R.string.app_name),
		 * "Please Enter Marks", false); return false; } }else{
		 * alertMessageForCharge(getString(R.string.app_name),
		 * "Please Enter Calculated Best of 5 GrandTotal before Submit", false);
		 * return false; }
		 */
		return true;
	}
	...
	private String row5Total_() {
		...
		if (!TextUtils.isEmpty(mark)) {
			if (Float.parseFloat(mark) > Float.parseFloat(maxTotalMark_5)) {
				View focusedView = getCurrentFocus();
				if (focusedView != null && focusedView instanceof EditText) {
					alertForInvalidMark(focusedView, true, "Total Exceeds "
							+ maxTotalMark_5);
		...
		if (!TextUtils.isEmpty(row_total4)) {
			...
			listTotalMarks.add(Float.valueOf("0"));
				...
				if (!listTotalMarks.isEmpty()) {
					...
					// 4 is compulsory
					float mark = Float.parseFloat(TextUtils.isEmpty(row_total5) ? "0" : row_total5);
...
*/
		...
		builder.setSingleChoiceItems(remarksList, -1,
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						et.setOnClickListener(Scrutiny_MarkDialog_R13_BTech_SpecialCase_New.this);
						et.clearFocus();
						dialog.dismiss();
	...
	public void afterTextChanged(Editable s) {
		...
		//	setTextToFocusedView(arg0.toString());
			...
			if (focusedView != null && focusedView instanceof EditText) {
				EditText et_focusedView = ((EditText) focusedView);
				et_focusedView.removeTextChangedListener(Scrutiny_MarkDialog_R13_BTech_SpecialCase_New.this);
				et_focusedView.setText("");
				et_focusedView.addTextChangedListener(Scrutiny_MarkDialog_R13_BTech_SpecialCase_New.this);
			}
			}
			}catch(Exception e){
				e.printStackTrace();
